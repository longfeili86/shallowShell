function Index = getIndexHalfdomain(nx,ny,grid,mixedBoundary)
...
% modified by H.Ji 12/25/15
    ...
    %% Index for mixed boundary conditions with left/right domain
%     Index.RTboundary = (ny*(nx+1)/2+1):ny:(ny*(nx-1)); Index.RTboundary = Index.RTboundary(:);
%     Index.RBboundary = (ny*(nx+1)/2+ny):ny:(ny*(nx-1)); Index.RBboundary = Index.RBboundary(:);
%     
    Index.RTboundary = (ny*(nx+1)/2+1-ny):ny:(ny*(nx-1)-ny+1); Index.RTboundary = Index.RTboundary(:);
    Index.RBboundary = (ny*(nx+1)/2):ny:(ny*(nx-1)); Index.RBboundary = Index.RBboundary(:);
    Index.Rboundary = (nx*ny-ny+2):(nx*ny-1); Index.Rboundary = Index.Rboundary(:);
    
    
    Index.RTCorner = nx*ny-ny+1;
    Index.RBCorner = nx*ny;

    Index.RboundaryAll = [Index.RTboundary;Index.RBboundary;...
       Index.Rboundary;Index.RTCorner;Index.RBCorner];

    Index.RTboundaryIn1 = (ny*(nx+1)/2+1+1):ny:(ny*(nx-1)-2*ny+2); Index.RTboundaryIn1 = Index.RTboundaryIn1(:);
    Index.RBboundaryIn1 = (ny*(nx+1)/2+ny-1):ny:(ny*(nx-1)-1-ny); Index.RBboundaryIn1 = Index.RBboundaryIn1(:);
    Index.RboundaryIn1 = (nx*ny-2*ny+3):(nx*ny-2-ny); Index.RboundaryIn1 = Index.RboundaryIn1(:);
    
    Index.RTCornerIn1 = nx*ny-2*ny+2;
    Index.RBCornerIn1 = nx*ny-ny-1;
    
    Index.RInBoundary = [Index.RTboundaryIn1;Index.RTboundaryIn1;Index.RBboundaryIn1];
    
    
    Index.Tsingular = ny*(nx+1)/2+1-ny; 
    Index.Bsingular = ny*(nx+1)/2;
    
    Index.TsingularAdjacent = ny*(nx+1)/2+1-2*ny;
    Index.BsingularAdjacent = ny*(nx+1)/2 - ny;    
    
   
                    
    Index.InBoundaryCornerR = [Index.RTboundaryIn1,ones(size(Index.RTboundaryIn1));...
                            Index.RBboundaryIn1,2*ones(size(Index.RBboundaryIn1));...
                        Index.RboundaryIn1,3*ones(size(Index.RboundaryIn1));
                        Index.RTCornerIn1,4;Index.RBCornerIn1,5];
