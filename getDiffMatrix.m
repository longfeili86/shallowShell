...
% Calculate the differentiation matrices using finite difference schemes
    ...
    Dpy = spdiags(ones(ny+4,1),1,ny+4,ny+4)- spdiags(ones(ny+4,1),0,ny+4,ny+4);  % backward fd
    Dmy = spdiags(ones(ny+4,1),0,ny+4,ny+4)- spdiags(ones(ny+4,1),-1,ny+4,ny+4); % centered fd
    D2 = spdiags(ones(nx+4,1),1,nx+4,nx+4)-2*spdiags(ones(nx+4,1),0,nx+4,nx+4)...
        + spdiags(ones(nx+4,1),-1,nx+4,nx+4); % centered fd for second x derivative
    D1 = spdiags(ones(nx+4,1),1,nx+4,nx+4)-spdiags(ones(nx+4,1),-1,nx+4,nx+4);% centered fd 
